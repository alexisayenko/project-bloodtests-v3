...
> **Status (2026-09-11): in progress.** The nav entry and a placeholder page
> shipped 2026-09-11 (305add0, 0a4e06a): "Hormonal Pathways" sits in
> `NAV_ITEMS` right after Monitoring Panels (nine items), routes to a bare
> `#pathways`, is blocked while reports have validation errors, and renders
> `HormonalPathwaysView.tsx` — the `PageHeader` banner (overline
> "Endocrinology", description "Biochemical pathways of hormones") over
> "Coming soon." Its sidebar icon is the custom `PathwaysIcon` (three linked
> hollow circles, `customIcons.tsx`) rather than a `lucide-react` one. Not
> built: everything else below — the `#pathways/<axis>` routes and axis tabs,
> the lazy-loaded view, the diagram, its data and the month stepper.
...
- **Hover a badge** → the badge, its association lines and the linked
  T bubbles, enzyme or labelled arrow (T/LH lights LH → ↑T with its "Leydig
  cells" label) highlight in **one highlight color** — a design
