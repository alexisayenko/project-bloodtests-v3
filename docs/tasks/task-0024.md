...
tissues and regions, and receptors — no node, shape, dot or icon for any of
them. Where an arrow acts is kept as **data**, not drawing
([2.1](#21-bands)).
...
- **No tissue, cell or receptor icon** (removed 2026-09-11, BX1): none of them
  is drawn ([2](#2-ontology)).
...
- **Drawn cells, tissues, regions and receptors** (BX1) — kept as per-arrow
  ...
  cells appear, as small text on their arrows. No tissue or receptor icon.
...
icons in the logo's line style — the woven-ball carriers, the enzyme ring,
flask / calculator / dashed circle, with no tissue or receptor icon — none on
...
- Browser check: "Hormonal Pathways" sits right after Monitoring Panels with
  ...
  SHBG, Albumin, enzyme, and the three provenance states (flask, calculator,
  dashed circle) — uses its custom icon from [2.8](#28-icons), in the logo's
  navy 2px round-capped line with no fills except the carriers' teal-filled
  node version; SHBG's woven ball holds one steroid deep in a gap and
  Albumin's two or three at its edge; there is no tissue, cell or receptor
  icon; the badge group headings carry no icon, and the banner no
  illustration.
