...
8. **The liver glyph.** `LiverIcon` is a blocky 32 × 27 outline scaled up to
   128px, and reads coarse at organ size; a finer drawing, artwork or data-free,
   is open.
...
- **Liver**: an organ node (128px) above VLDL with **HMG-CoA reductase** docked
  ...
  retrieved. The liver is `LiverIcon`, a blocky outline at 128px (open
  question 8).
...
- **Tests**: `lipoprotein-particles.json` against its schema and for internal
  ...
  blocking in `routing.test.ts`. Nothing renders the view in a test.
