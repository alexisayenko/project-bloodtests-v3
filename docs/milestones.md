...
- 2026-09-20 — Import and export become verbatim ([ADR-0028](tech/decisions/adr-0028-verbatim-import-export.md)): the stored per-report files are held as they arrived and returned byte for byte by the zip and the cloud push, and nothing is re-derived, whitelisted or rebuilt. A file is written only for a new upload or an edit, and stamped with `lastUpdatedDate` (schema 3.2; `generatedAt` deprecated). A push reads the cloud first and lays only the user's own changes over it, skipping any changed file that would lose a stored field. The unit checks read the stored `unit`, so an MCHC printed `%` with `unit: g/dL` no longer warns ([ADR-0025](tech/decisions/adr-0025-mchc-percent-is-not-an-accepted-unit.md)). This supersedes the export-side unit derivation of the 2026-09-09 entries below.
- 2026-09-11 — Hormonal Pathways runs on the user's own data ([task-0024](tasks/task-0024.md)): a ‹ date › stepper over the Hypogonadism panel's results-table dates (the shared `panelDates`, latest by default), readings in the SI/US system, Free T, Bio-T and the three ratios from the index engine, the SHBG- and albumin-bound pools from `testosteronePools`, and a "Use albumin 43 g/L (4.3 g/dL) when not measured" checkbox, on by default. Node captions become chips with status dots; chips and badges expand in place, one at a time, to a reference range judged as adult male — the lab's printed one, else cited ranges from the new `pathway-reference-ranges.json` (LHSC's lab test guide, Travison 2017, Swerdloff 2017; the major reference labs could not be retrieved, left open), `INDEX_DEFS`' zones for indices, none for the calculated pools. Estradiol joins `molar-masses.json` (18 entries), and an opened badge now veils the diagram while hover only draws its lines. The suite is 816 tests across 36 files.
- 2026-09-11 — Hormonal Pathways replaces "Coming soon." with a first static version of the male gonadal axis ([task-0024](tasks/task-0024.md)): four captioned zones on one canvas — Hypothalamus + Pituitary (still empty), Blood Transport with SHBG and albumin each holding docked T either side of free T, Testes with Sertoli and Leydig cells, Target tissues with 5α-reductase, aromatase, DHT, E2 and their receptors — joined by pathway arrows measured from the page, on hand-coded sample values. Six badges on the right (Total, Free and Bioavailable T, T/LH, DHT/T, T/E2) expand on click to meaning, low, high and caveats, and on hover draw a purple bus to the nodes each one reads. The build supersedes parts of the spec: zones rather than bands, one badge column, association lines hidden at rest. The desktop sidebar collapses to a 64px icon rail from a « / » toggle at its foot, remembered in this browser, with the tagline centered in its free space.
- 2026-09-11 — The Hormonal Pathways design is specified: [task-0024](tasks/task-0024.md), the [pathway](product/concepts/pathway.md) concept, and [ADR-0014](tech/decisions/adr-0014-pathway-wiring-is-mermaid-generated-to-json.md), which makes per-axis Mermaid wiring the source of a generated `pathways.json`. The section's nav entry ships the same day, ahead of the diagram: "Hormonal Pathways" (`#pathways`) is the ninth nav item, right after Monitoring Panels and blocked like it while reports have errors, over a "Coming soon." placeholder page. The desktop sidebar is now fixed under the top bar so it never scrolls — page and footer offset by its width, tighter items, the tagline hidden on short screens — and two custom icons replace lucide's: `PillIcon`, a split capsule, for Medications (and the Pancreatic Function card), and `PathwaysIcon`, three linked circles, for the new section. The suite is 808 tests across 36 files.
- 2026-09-11 — Hypogonadism gains bioavailable testosterone, computed with cFT's Vermeulen equation, and the first index judged by sex: its bands are Mayo Clinic Laboratories' reference limits for the sex set in Database details, and it carries no status until sex is set. Albumin printed in g/L now converts, which cFT had read as g/dL. The Reference Book adds a Testosterone page — secretion, plasma binding, conversion, feedback and clomiphene — with numbered links to quoted sources and analyte names that open their popups. A cleanup pass follows: a redirect off a blocked section no longer adds a history entry, so Back stops looping; month keys and labels move into one `data/months.ts`; and dead code goes — the stored schedule's `lab`, the per-tab control disabling and the 3D chart's unused window controls. The suite is 807 tests across 36 files.
- 2026-09-11 — The desktop screens are restyled after six generated mockups of the Monitoring Panels grid (`docs/brand/designs/`): design tokens in one `:root` block with a softer status palette, shared primitives in `web/src/components/primitives/`, and the grid, results table, controls bar, tabs, Panel Detail header, Diagnostic Reports, Scheduled Visits, Medications and Account moved onto cards. The grid gains status filter toggles and a remembered Compact view, and every name the app uses gets one term (`friendlyName`, `shortName`, `rawName`, …). The suite is 789 tests across 35 files.
...
- 2026-09-10 — All Observations' tabs become addressable (`#all/in-range`), and Generate Test Data lands there once the demo reports are in, so the first thing a new visitor sees is the chart.
...
- 2026-09-09 — A unit stops being one field (export-side derivation since
  retired by [ADR-0028](tech/decisions/adr-0028-verbatim-import-export.md):
  the stored pair is carried as it is).
  [task-0011](tasks/task-0011.md) closed: export then wrote the pair the
