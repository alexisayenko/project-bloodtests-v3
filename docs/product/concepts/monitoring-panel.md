...
The Monitoring Panels screen (`web/src/components/conditions/PanelsGridView.tsx`) renders each panel as a tinted card — icon, name, marker count and a one-line description — with its observations as chips, each carrying a small status dot (green in range, red out of range, amber with no reference range, grey never measured), and the panel's [computed indices](computed-index.md) as chips below a divider (same dot language, colored by ok/warn/bad zone); a search box above the grid narrows the cards by panel name, marker or index. Beside it, status filter toggles (`StatusFilterBar.tsx`, logic in `statusFilter.ts`) hide the chips of any status switched off, the count then reading "N of M markers", and a remembered "Compact view" (`compactPanels` in `bloodtests_view_settings_v1`) shows short names instead of friendly ones in narrower cards. Clicking a chip opens a popup with its detail; clicking the card's header or its "View panel →" link (absent in Compact view) opens the panel's detail view. The detail view's Results tab shows observations and indices in one table, adds a "Scheduled" toggle column to it (global set in localStorage, shared across panels) and marks the rows related to the selected one with a • beside the name.
