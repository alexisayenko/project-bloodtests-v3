...
the stored files' `sex` (read from the held files, first file that has one), else the device-local one in Database details (`bloodtests_envelope_meta_v1`); `birthYear` is not read.
