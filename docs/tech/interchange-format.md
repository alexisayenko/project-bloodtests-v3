...
> - **The app reads a subset.** Still **ignored for display**: `generatedAt` (deprecated, read without error or warning and passed through), `contentHash` (never verified), `subject`, `birthYear`, `notes`; per-report `issuedAt` and `specimen`; per-observation `interpretation` and `specimen`; range `appliesTo`/`ageLow`/`ageHigh`; and `label` and every band after the first bounded one survive only as display text. Ignored for display is not dropped: the held file still holds all of it.
...
| `3.2` | optional [`lastUpdatedDate`](#lastupdateddate); [`generatedAt`](#generatedat) deprecated (still valid, no longer written). Non-breaking: a `3.1` file reads unchanged, and only newly written files are stamped `3.2`. |
...
**Optional** — when absent, a reader must not assume a unit, and must not compare the number to a range in a different one. In particular it must not fall back to [`rawUnit`](#rawunit): an absent `unit` means the printed string could not be placed, which is exactly the case where treating it as a scale is unsafe.

The unit in canonical UCUM form: `mmol/L`, `mg/dL`, `10*9/L`. Printed and canonical are two different things, and this field holds the canonical one, with the printed string beside it in [`rawUnit`](#rawunit) — the product-level distinction is the [unit](../product/concepts/unit.md) concept, and the vocabulary is settled as UCUM in [ADR-0007](decisions/adr-0007-ucum-as-the-unit-vocabulary.md).
