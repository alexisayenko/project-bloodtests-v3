...
Machine-readable form of this page: [`https://paneloom.com/schema/bloodtests-3.schema.json`](https://paneloom.com/schema/bloodtests-3.schema.json) (source: `web/public/schema/bloodtests-3.schema.json`), JSON Schema draft 2020-12. It describes **major version 3 only** — every minor within it, and no other major — and so does the upload parser: the two agree, and nothing else is read; see [`schema`](#schema). The file name stays major-only and its `$id` never moves, because a minor is a backward-compatible addition and a reader that fetches this URL has to go on validating both older and newer 3.x files. Objects in it are deliberately open (no `additionalProperties: false`), because adding an optional field is not a breaking change here — it bumps the minor, and a reader ignores what it does not know — and [`identifiers`](#identifiers) is specified to pass unrecognised lab keys through. `web/test/envelope-schema.test.ts` holds it to the files the app writes and to this page's fields.

Schema ids moved from `blood.isayenko.net` to `paneloom.com` (same paths); the envelope's `schema` field is a `"major.minor"` string, never a URL, so old files still import.

> **Status: partially implemented.** This page is the full spec; the app implements a subset of it.
>
> - **Upload** (`web/src/data/parseUpload.ts`) accepts the v3 envelope and **nothing else** — no `schema: 1`, no project-bloodtests-v2 canonical draws, no legacy flat or grouped shapes; anything else fails with "Unrecognized JSON shape". Older files are converted first with `npm run convert:v3` (see [ADR-0009](decisions/adr-0009-v3-only-and-rawname.md)). It reads only `schema` (any `"3.x"`, plus the legacy number `3`; see [ADR-0012](decisions/adr-0012-envelope-version-is-a-major-minor-string.md)) and `diagnosticReports`. Per report it reads `lab`, `collectedAt` (date part only), `observations`, and `identifiers` — the first of `visit`/`order`/`accession` is folded into the session id so two same-day same-lab draws don't collide on merge (it isn't stored beyond that). Per observation it reads `loinc`, `rawName`, `value`, `comparator`, `rawValue`, `rawUnit` **or** `unit` — `rawUnit` is what the app displays and edits, while the unit checks (unit against the LOINC, the canonical form) read the stored `unit` first and fall back to `rawUnit` only when `unit` is absent — `method`, and `referenceRanges` — flattened to a single min/max pair plus display text. It also runs unit normalization over every observation on the way in (`web/src/data/unitNormalization.ts`), the one place in the app that does: the printed `value` and `unit` are kept exactly as read, and where the unit places cleanly and the conversion to the code's canonical UCUM unit is known, the derived pair is attached to the in-memory row as `canonical` — never written back to `value`/`unit`, and never exported. Still **ignored on upload** for display: `generatedAt`, `lastUpdatedDate`, `contentHash` (never verified), `subject`, `birthYear`, `notes` (`sex` feeds Database details); per-report `issuedAt` and `specimen`; per-observation `interpretation` and `specimen`; range `appliesTo`/`ageLow`/`ageHigh`; and `label` and every band after the first bounded one survive only as display text (the first printed `text`, or, when no range prints one, each band's label or `low-high` joined) — none of it is used for range selection.
> - **Export and import carry the stored files, not a rebuild** ([ADR-0028](decisions/adr-0028-verbatim-import-export.md)). The per-report JSON files in the private data repo are the truth. Import (zip, JSON upload, cloud pull) captures each file's text as it arrived and holds it next to the parsed display model; export (zip, cloud push) returns those texts byte for byte. Nothing re-derives `unit` from `rawUnit`, drops a field, rewrites `collectedAt`, re-hashes or re-serializes a file the app did not change, so `unit`, `interpretation`, `specimen`, `identifiers`, `comparator`, multi-band ranges, a legacy `generatedAt`, `contentHash` and the metadata fields all survive as stored, and the export → import → export cycle and the cloud pull → push cycle are byte-identical (a `web/test/verbatim-roundtrip.test.ts` invariant). A data error is fixed in the data repo, never by app logic.
> - **Where the app does write a file.** Three places only. (1) A new upload becomes one single-report envelope per report, each spreading the original report object and the envelope's own fields (no whitelist, no unit derivation), `reports/YYYY-MM-DD__<lab>.json`, two-space JSON plus a trailing newline, with `lastUpdatedDate` set; a single-report upload that already carries `lastUpdatedDate` keeps its own text. (2) An edit in the app patches only the touched observation fields of that one file, refreshes `lastUpdatedDate`, and refreshes `contentHash` only if the file already had one; the file is re-serialized (so `4.50` becomes `4.5` in that file only). (3) A session with no stored file (generated test data, or sessions kept by an older build) gets one file built once, carrying `rawUnit` and no `unit`; that file serves local display and export only, and is never pushed over a file the cloud already has (a push sends only what the user changed, each file lossless: [ADR-0028](decisions/adr-0028-verbatim-import-export.md)). The three local files are rebuilt from browser storage once they change; while unchanged they are the held texts. A cloud folder with no `manifest.json` gets one on the first push.
> - **The app reads a subset.** Still **ignored for display**: `generatedAt` (deprecated, read without error or warning and passed through), `contentHash` (never verified), `notes`; `subject`, `sex` and `birthYear` are shown read-only in Database details, read from the held files, never written back; per-report `issuedAt` and `specimen`; per-observation `interpretation` and `specimen`; range `appliesTo`/`ageLow`/`ageHigh`; and `label` and every band after the first bounded one survive only as display text. Ignored for display is not dropped: the held file still holds all of it.


### Known round-trip gaps

Before ADR-0028 the exporter rebuilt every file field by field, and each of these was a loss: a `comparator`, `identifiers`, `interpretation`, `specimen`, text-only ranges and a hand-corrected `unit` did not survive export. **They no longer do, for any file that was imported or pulled and not edited**: the file leaves as it arrived. What remains:
...
- **`npm run convert:v3` drops a decimal comma rather than reading it.** The converter coerces each `value`, `refMin` and `refMax` with `Number()` and keeps the result only when finite, so a legacy value written `1,5` becomes `null` instead of `1.5`. A row that loses its `value` this way and carries no `rawValue` then fails validation as a missing result value. The chatbot prompt asks for decimal commas to be normalized at the source; the converter does not do it itself.
...
Eight keys: `schema` and `diagnosticReports` are required, `lastUpdatedDate`, `contentHash`, `subject`, `sex`, `birthYear` and `notes` are optional. The deprecated `generatedAt` may also appear in a legacy file. This page specifies the envelope only.
...
| `3.1` | an observation's unit leaves as a **pair** — [`unit`](#unit) holds the printed spelling folded to its UCUM code, [`rawUnit`](#rawunit) the string the lab printed. Recorded retroactively: an earlier exporter wrote it; the app now carries both fields as stored and derives neither. |
| `3.2` | optional [`lastUpdatedDate`](#lastupdateddate); [`generatedAt`](#generatedat) deprecated (still valid, no longer written). Non-breaking: a `3.1` file reads unchanged. Only a file the app builds for a session with no stored file is stamped `3.2`; an uploaded file keeps the `schema` it arrived with, and an edit changes no envelope field but `lastUpdatedDate`. |

The value lives in one place — `web/src/data/envelopeSchema.ts` (`SCHEMA_VERSION`, over `SCHEMA_MAJOR`) — read by the file writers, the upload parser and both offline scripts alike. An imported file keeps the `schema` it arrived with.

## `lastUpdatedDate`

**Optional**, camelCase, added in `3.2`. Full ISO 8601 UTC timestamp — `T` separates date and time, `Z` means UTC.

When the data in this file was last modified by the app. It is set or refreshed **only** when report data is modified: a new report created from an upload, or an existing report edited. It is never set by importing, pulling or exporting an unmodified file, so a file that only travelled keeps whatever it had — absent stays absent, present stays verbatim. When absent, a reader treats the modification time as unknown.

## `generatedAt`

**Deprecated** (`3.2`): no use case was found for a generation time, and the app no longer writes it. It stays in the schema as an optional property so a legacy file that carries it still validates; the app reads such a file with no error or warning and passes the field through verbatim, never stripping it. [`lastUpdatedDate`](#lastupdateddate) is the only timestamp field.
...
Same encoding as the envelope's [`lastUpdatedDate`](#lastupdateddate) — full ISO 8601 UTC timestamp.
...
**UCUM** — [Unified Code for Units of Measure](https://unitsofmeasure.org), the standard FHIR uses for units. [`unit`](#unit) holds a UCUM code, and [`rawUnit`](#rawunit) holds the string the lab printed, so the file records both what the report said and the scale it says it on. Normalizing the spelling is not converting the number: the value is exactly the one the lab printed, and a printed unit the curated tables cannot place leaves `unit` absent rather than guessed. See [ADR-0007](decisions/adr-0007-ucum-as-the-unit-vocabulary.md) and the [unit](../product/concepts/unit.md) concept.
...
**Optional** — when absent, a reader must not assume a unit, and must not compare the number to a range in a different one. In particular it must not fall back to [`rawUnit`](#rawunit) to compare or convert numbers: an absent `unit` means the printed string could not be placed, which is exactly the case where treating it as a scale is unsafe. (The app reads `rawUnit` only as the display string and, when `unit` is absent, as the input of its own unit checks; it derives no unit and stores none.)

The unit in canonical UCUM form: `mmol/L`, `mg/dL`, `10*9/L`. Printed and canonical are two different things, and this field holds the canonical one, set by the data's owner, with the printed string beside it in [`rawUnit`](#rawunit) — the product-level distinction is the [unit](../product/concepts/unit.md) concept, and the vocabulary is settled as UCUM in [ADR-0007](decisions/adr-0007-ucum-as-the-unit-vocabulary.md).

Only the **spelling** is normalized. `ммоль/л`, `mmol/l` and `mmol/L` all read as `mmol/L`; no conversion happens here, so the number this labels is still the number the lab printed. Converted values belong nowhere in this file ([ADR-0003](decisions/adr-0003-store-only-what-the-lab-printed.md)) — a reader that wants other units converts at display time, from the reported pair.

The app never fills or changes it: it is stored data, carried verbatim ([ADR-0028](decisions/adr-0028-verbatim-import-export.md)), so a `unit` corrected by hand in the data repo (as [ADR-0025](decisions/adr-0025-mchc-percent-is-not-an-accepted-unit.md) does) survives every export. Only a file the app itself builds for a session with no stored file carries `rawUnit` and **no** `unit`: deriving one would claim a normalization the lab never printed. `ucumUnitFor` in `web/src/data/unitNormalization.ts` remains the display-time fold.
...
**Optional** — written whenever the report stated a unit at all, and never rewritten. Absent means the report printed no unit.

The unit exactly as the lab printed it: `мкМЕ/мл`, `mmol/l`, `×10⁹/л`. It is the provenance half of the pair — [`unit`](#unit) holds the normalized form, so the normalization is auditable and reversible rather than lossy, and the printed string survives a round trip that folds four spellings into one code.

Upload displays and edits it in preference to `unit`, but validates against `unit` when the file has one (a `%` printed on an MCHC line and a stored `g/dL` raises no warning, [ADR-0025](decisions/adr-0025-mchc-percent-is-not-an-accepted-unit.md)); a file that carries only `rawUnit`, or only `unit` (chatbot output, a hand-written file) is read from there instead. The canonical form is UCUM — decided in [ADR-0007](decisions/adr-0007-ucum-as-the-unit-vocabulary.md), with the product-level reasoning on the [unit](../product/concepts/unit.md) concept page and the work tracked as [task-0011](../tasks/task-0011.md).
