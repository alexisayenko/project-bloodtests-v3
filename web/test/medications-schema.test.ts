...
import { addRow, emptyMedications, toggleMonth, updateRow, type Medications } from '../src/data/medications';
...
describe('published JSON Schema — medications-1.schema.json', () => {
  ...
  it('accepts a row carrying a name, a dosage and some months', () => {
    let meds = addRow(emptyMedications(2026), 'row-1');
    meds = updateRow(meds, 'row-1', { name: 'Levothyroxine', dosage: '50mcg' });
  ...
  it('accepts a hand-built example with an empty dosage and no months yet', () => {
    const meds: Medications = {
      years: [2025, 2026],
      rows: [{ id: 'row-2', name: 'Vitamin D', dosage: '', months: [] }],
    };
  ...
  it('rejects a row with an empty id', () => {
    const meds: Medications = { years: [2026], rows: [{ id: '', name: 'X', dosage: '', months: [] }] };
    expect(errorsFor(meds)).not.toEqual([]);
  });

  it('rejects a month key that is not ISO YYYY-MM', () => {
    const meds: Medications = { years: [2026], rows: [{ id: 'row-3', name: 'X', dosage: '', months: ['2026-1'] }] };
    expect(errorsFor(meds)).not.toEqual([]);
  });
