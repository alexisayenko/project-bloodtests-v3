...
/** Synchronous SHA-256 of the UTF-8 bytes of `text`, as lowercase hex. */
export function sha256Hex(text: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) compress(h, padded, offset, w);
  return Array.from(h, (n) => n.toString(16).padStart(8, '0')).join('');
}

/** `sha256:` plus the hex digest of `JSON.stringify(diagnosticReports)` (the envelope's `contentHash`). */
export function contentHashOf(reports: readonly InterchangeReport[] | readonly unknown[]): string {
  return `sha256:${sha256Hex(JSON.stringify(reports))}`;
}
