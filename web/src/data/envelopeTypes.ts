...
/**
 * The blood-tests interchange envelope, major version 3 (currently 3.2). Machine-readable form of docs/tech/interchange-format.md: the JSON object that carries a set of lab reports for one person between systems. This schema describes MAJOR version 3 and every minor within it — the file name stays major-only (bloodtests-3.schema.json) and its $id never moves, because a minor is by definition a backward-compatible addition and a reader that fetches this URL must go on validating both older and newer 3.x files. Major 3 is the only version there is: the parser at paneloom.com accepts nothing else, and a file stamped with the earlier 1 is converted offline with `npm run convert:v3` before it can be read. Objects are deliberately OPEN (no `additionalProperties: false`): the prose spec states that adding an optional field is not a breaking change — it bumps the minor only, and a reader ignores what it does not know — and `identifiers` is specified to pass unrecognised lab keys through under the lab's own name — closing the objects would make every such forward-compatible file invalid against a schema that was never meant to reject it. Validation here is therefore about the fields the format defines, not about forbidding fields it has not defined yet. Fields the app does not yet read or write are still specified: the format is the contract, the implementation is a subset of it.
 */
export interface InterchangeEnvelope {
  /**
   * Required. The format version as the STRING "major.minor" — "3.2" today. A string rather than a number because JSON numbers cannot tell 3.10 apart from 3.1, and there will be a tenth minor. The MAJOR answers the reader's one question — can I read this? — and is bumped only on a breaking change (a field removed, renamed, or given a new meaning); major 3 is accepted and every other major is refused. The MINOR is bumped on every other change to the envelope format, each of which is a backward-compatible addition: a 3.0 file still loads under a reader written for 3.1, and a 3.2 file loads under that same reader, because the objects here are open and unknown fields are ignored. The bare NUMBER 3 is the one legacy spelling still accepted, read as 3.0 — it is what files written before the version became a string carry. The bare string "3" is not accepted: the string form always carries a minor. A file stamped 1 is an older format that has to be converted with `npm run convert:v3` before it can be read.
   */
  schema: 3 | string;
  /**
   * @deprecated
   * Deprecated; kept so files written by earlier versions still validate. Full ISO 8601 UTC timestamp of when the file was written. The app no longer writes it (lastUpdatedDate is the only timestamp field), but a file that carries it is read without complaint and passed through untouched on export and sync. A reader ignores it.
   */
  generatedAt?: string;
  /**
   * Optional. Full ISO 8601 UTC timestamp of the last time the app modified the report data in this file: set when the app creates the file from an upload and refreshed when a report in it is edited. Never set or refreshed by reading, exporting or syncing a file unchanged, so an untouched file keeps its bytes. When absent, the file has not been modified since it was created elsewhere.
   */
