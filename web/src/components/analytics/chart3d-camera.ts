...
export interface OrbitCamera3DOptions {
  ...
  pitchMax: number;
  cameraDist?: number;
  // Also gates double-tap-to-reset and the `touch-action: none` override.
  enableZoom?: boolean;
  defaultZoom?: number;
  zoomMin?: number;
  zoomMax?: number;
...
export const createOrbitCamera3D = (
  ...
  // Eased rather than jumped so the room never snaps disorientingly.
  ...
  // Locked ignores user input only; goTo/resetView still move the camera.
  let locked = false;
  ...
  if (enableZoom) {
    // Pinch needs raw two-finger pointer events, not native gesture handling.
