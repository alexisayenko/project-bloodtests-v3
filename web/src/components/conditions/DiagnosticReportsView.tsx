...
const td = { padding: '8px 12px', borderBottom: `1px solid ${COLOR.borderSubtle}`, whiteSpace: 'nowrap' } as const;
...
export function DiagnosticReportsView({
  ...
  const addBlock = (
    ...
    >
      ...
      <div style={{ color: COLOR.textSecondary, fontSize: 14 }}>
        ...
        <label
          style={{
            ...
            padding: '2px 14px',
            fontSize: 13,
            border: `1.5px solid ${isAdding ? COLOR.border : COLOR.accent}`,
            color: isAdding ? COLOR.textMuted : COLOR.accent,
      ...
      {addError && <div style={{ color: COLOR.statusBad, fontSize: 14, marginTop: 12 }}>{addError}</div>}
      {addedCount != null && (
        <div style={{ color: COLOR.statusOk, fontSize: 14, marginTop: 12 }}>
                  ...
                  <tr key={group.file} {...pressable(() => onOpenDetail(group.file))} style={{ cursor: 'pointer' }}>
                    <td style={td}>
                      {groupHasErrors(group.file, issues) && (
                        <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', backgroundColor: COLOR.statusBad }} title="Errors" />
                      )}
                      {!groupHasErrors(group.file, issues) && groupHasWarnings(group.file, issues) && (
                        <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', backgroundColor: COLOR.statusWarn }} title="Warnings" />
                      )}
                    </td>
        ...
        >
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <label style={{ ...ACTION, border: `1.5px solid ${COLOR.accent}`, color: COLOR.accent }}>
                Import JSON{' '}
                <input
                  type="file"
                  accept=".json,application/json"
            ...
            {importError && <div style={{ color: COLOR.statusBad, fontSize: 14, marginTop: 12 }}>{importError}</div>}
          </div>
          <div style={{ borderTop: `1px solid ${COLOR.borderSubtle}`, marginTop: 16, paddingTop: 16 }}>
            ...
            <div
              ...
              style={{ ...ACTION, border: `1.5px solid ${COLOR.statusBad}`, color: COLOR.statusBad }}
            >
              Clear
            </div>
          </div>
