...
import { DEFAULT_ALBUMIN_GDL, TESTOSTERONE_MOLAR_MASS, testosteronePools } from '../../data/indexDefs';
...
import {
  ...
  type FeedbackMark,
  type PathwayEdge,
} from './HormonalPathways.geometry';
import {
  BADGES,
  CALCULATED_FREE_T,
  CAPTIONS,
...
/** A chip's floating card — a caption's or a receptor's effects — as opposed to a badge. */
const isCardId = (id: string | null) => isCaptionId(id) || effectsNodeOf(id) !== null;
...
function Caption({ id }: Readonly<{ id: CaptionId }>) {
  ...
  return (
    <button
      ...
      onClick={(e) => state.toggleChip(id, e.currentTarget)}
    >
...
const PATHWAYS: readonly PathwayEdge[] = [
  ['pituitary', 'fsh', 'fork'],
  ['pituitary', 'lh', 'fork'],
  ['lh', 'leydig', 'straight'],
...
function PathwayArrows({ root, active, focused, layoutKey }: Readonly<{ root: RefObject<HTMLDivElement | null>; active: string | null; focused: string | null; layoutKey: string }>) {
  ...
  const measure = (el: HTMLDivElement) => {
    ...
    setLines([
      ...
      ...buildPathwayLines(el, base, centerX, PATHWAYS),
      ...feedback.lines,
    ]);
  };
...
function FreeTestosteroneBody({ badge, scope, snapshot, unitSystem }: Readonly<{ badge: Badge; scope: string; snapshot: Snapshot; unitSystem: UnitSystem }>) {
  ...
  const pctRef: ReferenceInfo = {
    tag: indexDefOf('ftpct')?.evidenceLevel,
...
function badgeMeasure(b: Badge, snapshot: Snapshot): Measure | undefined {
  return b.unavailable || !b.measure ? undefined : snapshot[b.measure];
}

function BadgeBody({ badge, scope, snapshot, unitSystem }: Readonly<{ badge: Badge; scope: string; snapshot: Snapshot; unitSystem: UnitSystem }>) {
  if (badge.id === FREE_T_BADGE) return <FreeTestosteroneBody badge={badge} scope={scope} snapshot={snapshot} unitSystem={unitSystem} />;
  const measure = badgeMeasure(badge, snapshot);
  const info = measure ? referenceOf(badge.measure!, snapshot, unitSystem) : NO_REFERENCE;
  return (
    <div className="mc-pathway-badge-body">
      <ReferenceBlock scope={scope} info={info} />
      <span><b>Meaning</b> {badge.meaning}</span>
...
export function HormonalPathwaysView({
  ...
  const page = useAxisPage({
    ...
    allResults,
    rootRef: bandsRef,
    keepSelector: '.mc-pathway-pop, [data-caption], [data-badge], [data-effects]',
    isCardId,
  });
  ...
  const state = useMemo<PathwayState>(() => ({ open, snapshot, toggleChip }), [open, snapshot, toggleChip]);
  return (
    <div>
      <PageHeader
      ...
      <BadgeColumn
        ...
        leading={<PoolsDonut snapshot={snapshot} />}
        renderFace={(b) => {
          const measure = badgeMeasure(b, snapshot);
          return { status: measure?.status ?? 'none', value: measure ? valueText(measure) : 'Not available' };
        }}
